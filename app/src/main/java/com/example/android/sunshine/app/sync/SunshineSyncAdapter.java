...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    //Added for wearable watchface
    private GoogleApiClient mGoogleApiClient;

    public SunshineSyncAdapter(Context context, boolean autoInitialize) {
        super(context, autoInitialize);

        //create the connection to the wearable
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            for(int i = 0; i < weatherArray.length(); i++) {
                ...
                //Only send to the watch if its the newest info
                if(i == 0) {
                    getWeatherInfoForWatch(high, low, weatherId);
                }
            }
    ...
    //Created to take all information needed for the watch to package as an item
    public void getWeatherInfoForWatch(double high, double low, int weatherId) {

        Log.d(LOG_TAG, "Getting info for watch");

        if(mGoogleApiClient == null) {
            //if the client is empty no watch is connected so dont do anything
            return;
        }
                ...
                .setResultCallback(new ResultCallback<DataApi.DataItemResult>() {
                    ...
                    public void onResult(DataApi.DataItemResult dataItemResult) {
                        if(!dataItemResult.getStatus().isSuccess()) {
                            Log.e(LOG_TAG, "Data item failed to reach wearable.");
                        } else {
                            Log.d(LOG_TAG, "Data item reached wearable.");
                        }
                    }
                });
    }
